                ...
                <div class="container-xxl flex-grow-1 container-p-y">
                    <div class="row">
                        @foreach($restaurants as $restaurant)
                            <a href="{{route('all_orders.show',$restaurant->id)}}" class="card" style="width: 18rem;background-color: #5f61e6;margin-right: 10px;margin-bottom: 10px">
                                <div class="card-body" >
                                    <h5 class="card-title text-center" style="font-size: 25px;font-weight: bold;color:white">{{$restaurant->restaurant_name}}</h5>
                                    <div class="row">
                                        <h6 class="text-dark text-center">orders <span style="margin-left: 20px;color:white">{{$restaurant->orders->count()}}</span></h6>
                                    </div>
                                </div>
                            </a>
                        @endforeach
